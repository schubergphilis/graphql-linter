// Package validation provides type validation functionality for GraphQL schemas
package data

import (
	"github.com/schubergphilis/graphql-linter/internal/app/graphql-linter/data/ast"
	gqlast "github.com/wundergraph/graphql-go-tools/v2/pkg/ast"
)

// ValidateDataTypes checks if all field types in the schema are defined
func (s Store) ValidateDataTypes(doc *gqlast.Document, schemaContent string, schemaPath string) (bool, []int) {
	builtInTypes := ast.GetBuiltInTypes()
	definedTypes := ast.GetDefinedTypes(doc)
	...
	for _, objType := range doc.ObjectTypeDefinitions {
		for _, fieldRef := range objType.FieldsDefinition.Refs {
			...
			baseTypeName := ast.GetBaseTypeName(doc, doc.Types[field.Type])
...
func ValidateDirectiveNames(doc *gqlast.Document) bool {
...
func ValidateDirectives(doc *gqlast.Document, directiveRefs []int, validDirectives map[string]bool, parentName, parentKind string) bool {
...
func (s Store) ValidateEnumTypes(doc *gqlast.Document, schemaContent string, schemaPath string) ([]string, []int) {
